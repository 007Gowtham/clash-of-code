...
import { TerminalSquare, Settings, Braces, RotateCcw, ChevronDown, Play, Lock, X, CheckCircle, AlertCircle, Loader2, Crown, Code2, Trash2 } from 'lucide-react';
    ...
    <div className="flex-1 flex flex-col bg-white relative" style={style}>
      ...
      <div className="h-10 border-b border-gray-200 bg-gray-50/50 flex items-center justify-between px-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="p-1 hover:bg-gray-200 rounded text-gray-400 hover:text-gray-600 transition-colors" title="Editor Settings">
            <Settings className="w-3.5 h-3.5" />
          </button>
        </div>
      </div>

      {/* Code Area with Lock Overlay */}
      <div className="flex-1 flex flex-col relative">
        ...
        {/* {isLocked && (
          <div className="absolute inset-0 z-50 flex items-center justify-center bg-white/95 backdrop-blur-sm">
            <div className="text-center">
              ...
              <p className="text-gray-900 font-medium mb-2">Editor Locked</p>
              <p className="text-sm text-gray-600">
                {lockMessage || (selectedQuestion
                  ? 'Request assignment from team leader to unlock'
                  : 'Select a question and get it assigned to unlock the editor')}
              </p>
            </div>
          </div>
        )} */}
        ...
        <div className={`flex-1`}>
          <MonacoEditor
            height="100%"
            language={language}
            value={` ${definition ? definition : ''} ${code ? code : ''}`}
            onChange={(e) => {
              // Update state immediately for smooth typing
              if (setCode && e.target.value !== undefined) {
      ...
      {/* Output Drawer - Sliding from Bottom */}
      <div
        className={`absolute bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-xl transition-transform duration-300 ease-in-out z-40 flex flex-col ${drawer.isOpen ? 'translate-y-0' : 'translate-y-full'
          }`}
        style={{ height: '40%' }}
      >
        {/* Drawer Header */}
        <div className="flex items-center justify-between px-4 py-2 border-b border-gray-100 bg-white sticky top-0 z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {!isLoading && drawer.mode !== 'run' && (
                <p className="text-[9px] text-gray-500 font-medium uppercase tracking-wider mt-0.5">
                  Accepted
                </p>
              )}
            </div>
          ...
          {drawer.loading ? (
            ...
            <div className="flex flex-col h-full">
              {drawer.mode === 'run' ? (
                ...
                // Submission View with Premium Animations
                <div className="flex flex-col items-center justify-center h-full animate-in fade-in zoom-in-95 duration-500">
                      ...
                      {submissionResult?.firstFailure && (
                        <div className="bg-rose-50 border border-rose-200 rounded-lg p-4 max-w-md">
                          <p className="text-sm text-rose-700 font-medium">
                            Failed on test case {submissionResult.firstFailure.index + 1}
                          </p>
                          {submissionResult.firstFailure.testCase?.error && (
                  ...
                  <div className="mt-6 flex gap-3 opacity-0 animate-in fade-in slide-in-from-bottom-4 duration-700 fill-mode-forwards" style={{ animationDelay: '200ms' }}>
                    <button className="px-5 py-2 bg-gray-900 text-white rounded-lg text-[11px] font-bold hover:bg-black transition-colors shadow-lg shadow-gray-900/20 active:translate-y-px">
                      Next Problem
                    </button>
                    <button className="px-5 py-2 bg-white text-gray-600 border border-gray-200 rounded-lg text-[11px] font-bold hover:bg-gray-50 hover:text-gray-900 transition-colors">
                      View Solution
                    </button>
                  </div>
                </div>
